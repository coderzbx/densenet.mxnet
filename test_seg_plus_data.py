...
if not os.path.exists("./test_plus_batch_2"):
    os.makedirs("./test_plus_batch_2")
...
while next_batch:
    ...
    if batch_index == 0:
        for i in range(batch_size):
            ...
            # print("data:{}".format(data.shape))
            _label = data_label[i].asnumpy().astype(np.uint8)
            cv2.imwrite("./test_plus_batch_2/{}.png".format(image_index), _label)
            # label
            height = _label.shape[0]
            width = _label.shape[1]
            blank_image = np.zeros((height, width, 3), np.uint8)
            for label in self_road_chn_labels:
                ...
                # color = color[::-1]
                blank_image[np.where((_label == label.id))] = color
            cv2.imwrite("./test_plus_batch_2/{}-label.png".format(image_index), blank_image)

            image_label = data[i].asnumpy().astype(np.uint8).transpose(1, 2, 0)
            # image = image_label[:, :, 0:3]
            image = image_label[:, :, ::-1]
            cv2.imwrite("./test_plus_batch_2/{}.jpg".format(image_index), image)
