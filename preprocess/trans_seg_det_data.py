...
def make_list_dir(args):
    ...
    class_id = 0
    for package_dir in dir_list:
        file_list = os.listdir(os.path.join(dir_path, package_dir))
        for file_id in file_list:
            if len(file_id) < 4 or file_id[-3:] not in ['jpg']:
                continue

            real_path = os.path.join(dir_path, package_dir, file_id)
            label_path = os.path.join(dir_path, package_dir, file_id[:-3]+"png")
            ...
            image_label_path = os.path.join(dir_path, "../merge_data", file_id[:-3]+"png")
            cv2.imwrite(image_label_path, image_label)
            ...
            image_list.append((image_index, image_label_path, str(map_id)))
            image_index += 1
...
def image_encode(args, i, item, q_out):
    ...
    try:
        s = mx.recordio.pack_img(header, img, quality=args.quality, img_fmt=".png")
        q_out.put((i, s, item))
    except Exception as e:
        traceback.print_exc()
