...
if __name__ == "__main__":
    ...
    parser.add_argument('--scale_dir', type=str, required=True)
    ...
    if not os.path.exists(scale_dir):
        print("dir[{}] is not exist".format(scale_dir))
        exit(0)
    ...
    for class_id in class_dir_list:
        ...
        if len(file_list) > 5000:
            ...
            # scaled
            scale_class_dir = os.path.join(scale_dir, class_id)
            scale_list = os.listdir(scale_class_dir)
            for scale_id in scale_list:
                if len(scale_id) < 4 or scale_id[-3:] not in ['jpg', 'png']:
                    scale_list.remove(scale_id)
                    continue
            result_list.extend([os.path.join(scale_class_dir, scale_id) for scale_id in scale_list])
